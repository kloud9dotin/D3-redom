...
var d3svg = d3.select("#D3-render").append("svg")
...
d3.tsv("./data.tsv").then(function(data) {
 ...
 var brush = d3.brushX()//for slider bar at the bottom
    .extent([[xScale2.range()[0], 0], [xScale2.range()[1], 50]])
  ...
  context.append("path") // Path is created using d3svg.area details
    .attr("class", "area")
  ...
  var issue = d3svg.selectAll(".issue")
      .data(categories) // Select nested data and append to new d3svg group elements
    .enter().append("g")
      .attr("class", "issue");   
  ...
  // Hover line 
  var hoverLineGroup = d3svg.append("g") 
  ...
  function brushed() {
    ...
    d3svg.select(".x.axis") // replot xAxis with transition when brush used
          .transition()
          .call(xAxis);
    ...
    d3svg.select(".y.axis") // Redraw yAxis
      .transition()
      .call(yAxis);   
